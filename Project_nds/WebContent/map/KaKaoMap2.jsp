...
function makeMap(hospital_info){
    ...
    for (var i = 0; i < hospital_info.length; i ++) {
       ...
       geocoder.addressSearch(hospital_info[i].address, info(hospital_info[i].name, hospital_info[i].tel));         
    }
    
    function info(name, tel){//좌표 검색과, 해당 좌표에 대한 데이터를 출력하기 위한 클로저 함수
            ...
            if (status === kakao.maps.services.Status.OK) {
             ...
             kakao.maps.event.addListener(marker,'mouseout', function() {
               ...
               kakao.maps.event.addListener(marker, 'click', function() {
                  ...
                  location.href = "hospital_info.jsp?name=" +name+"&&tel="+tel;
               });
